...
def download_atpack_file(
    filename: str, info: Dict, atpacks_dir: Path, force: bool = False
) -> bool:
...
Examples:
  ...
  %(prog)s --output ./custom-dir    # Download to custom directory
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available AtPack files and exit"
    )
    ...
    parser.add_argument(
        "--output", "-o", type=Path, help="Output directory (default: atpacks/)"
    )
