#!/usr/bin/env python3
"""
Test the device specifications extractor with shadowidref handling

This test verifies that the device specifications extractor correctly
...
def test_shadowidref_handling():
    """Test that shadowidref attributes are properly handled."""
    print("🧪 Testing shadowidref handling in device specifications extraction")
    print("=" * 70)
    ...
    try:
        ...
        # Test devices that are known to have shadowidref attributes
        test_devices = ["PIC16F877A", "PIC16F84A", "PIC16F628A", "PIC16F688"]
        ...
        for device_name in test_devices:
            ...
            try:
                ...
                for sector in specs.gpr_sectors:
                    ...
                    print(f"      - {sector.name}: {addr_range} ({sector.size} bytes) [Bank {sector.bank}]")
                
                # Verify the specs make sense
                if specs.maximum_ram_size == 0:
                    print("   ⚠️  Warning: No RAM detected (possible shadowidref issue?)")
                elif specs.maximum_ram_size != specs.gpr_total_size:
                    print("   ⚠️  Warning: RAM size mismatch between maximum_ram_size and gpr_total_size")
                else:
                ...
                if specs.config_size > 0:
                    print(f"   ⚙️  Config: {specs.config_size} bytes @ {specs.config_addr}")
...
def compare_with_reference():
    ...
    csv_path = Path(__file__).parent.parent.parent / "atpack-python-get-specs" / "pic16_device_specs.csv"
    ...
    try:
        ...
        # Test a few key devices
        test_devices = ["PIC16F877A", "PIC16F84A", "PIC16F628A", "PIC16F688", "PIC16F883"]
        matches = 0
        mismatches = 0
        ...
        for device_name in test_devices:
            ...
            try:
                ...
                if device_matches == len(comparisons):
                    ...
                    mismatches += 1
                    print(f"   ⚠️  {device_matches}/{len(comparisons)} values match for {device_name}")
                
            except Exception as e:
                print(f"   ❌ Failed to extract specs for {device_name}: {e}")
                mismatches += 1
        
        print(f"\n📈 Summary: {matches} perfect matches, {mismatches} mismatches out of {len(test_devices)} devices tested")
        
        if matches == len(test_devices):
            print("🎉 All tests passed! shadowidref handling is working correctly.")
        elif matches > mismatches:
            print("✅ Most tests passed. shadowidref handling appears to be working.")
        else:
            print("⚠️  Many mismatches detected. May need to review shadowidref handling.")
