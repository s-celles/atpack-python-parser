...
def demonstrate_complete_extraction():
    ...
    try:
        ...
        for i, spec in enumerate(all_specs[:5]):
            ...
            print(f"  {i+1}. {spec.device_name:<12} - Flash: {spec.maximum_size:4d}W, RAM: {spec.maximum_ram_size:3d}B, EEPROM: {eeprom_info:<5}, GPR Banks: {gpr_banks}")
        print()
...
def show_cli_usage():
    ...
    print("  atpack devices specs PIC16F877A path/to/atpack.atpack --format json --output specs.json")
    print()
    print("Export to CSV:")
    print("  atpack devices specs PIC16F877A path/to/atpack.atpack --format csv --output specs.csv")
    print()
