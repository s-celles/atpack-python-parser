...
class TestPIC24FParsing:
    ...
    def test_pic24f_parser_initialization(self, pic24f_atpack_file):
        """Test that PIC24F AtPack can be initialized correctly."""
        parser = AtPackParser(pic24f_atpack_file)
        assert parser is not None
        assert parser.device_family == DeviceFamily.PIC

    def test_pic24f_metadata(self, pic24f_atpack_file):
    ...
    def test_pic24f_device_list(self, pic24f_atpack_file):
    ...
    def test_pic24f16ka301_device_info(self, pic24f_atpack_file):
    ...
    def test_pic24f16ka301_memory_segments(self, pic24f_atpack_file):
        ...
        assert any("PROG" in name.upper() for name in segment_names)  # PIC24F uses "PROG" segments
        assert any("SFR" in name.upper() for name in segment_names)   # Special Function Registers

    def test_pic24f16ka301_registers(self, pic24f_atpack_file):
    ...
    def test_pic24f16ka301_config(self, pic24f_atpack_file):
    ...
    def test_pic24f_invalid_device(self, pic24f_atpack_file):
...
class TestPIC24FCLI:
    ...
    def test_pic24f_files_info_cli(self, pic24f_atpack_file):
        """Test files info command for PIC24F AtPack."""
        runner = CliRunner()
        result = runner.invoke(app, ["files", "info", str(pic24f_atpack_file)])
    ...
    def test_pic24f_devices_list_cli(self, pic24f_atpack_file):
        """Test devices list command for PIC24F AtPack."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "list", str(pic24f_atpack_file)])
        
        assert result.exit_code == 0
        assert "PIC24F16KA301" in result.stdout
        assert "🟡" in result.stdout or "[PIC]" in result.stdout

    def test_pic24f_devices_list_json_cli(self, pic24f_atpack_file):
        """Test devices list command with JSON output for PIC24F AtPack."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "list", str(pic24f_atpack_file), "--format", "json"])
    ...
    def test_pic24f_devices_search_cli(self, pic24f_atpack_file):
        """Test device search command for PIC24F devices."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "search", "PIC24F16KA*", str(pic24f_atpack_file)])
        
        assert result.exit_code == 0
        assert "PIC24F16KA301" in result.stdout

    def test_pic24f_device_info_cli(self, pic24f_atpack_file):
        """Test device info command for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "info", "PIC24F16KA301", str(pic24f_atpack_file)])
    ...
    def test_pic24f_device_info_json_cli(self, pic24f_atpack_file):
        """Test device info command with JSON output for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "info", "PIC24F16KA301", str(pic24f_atpack_file), "--format", "json"])
    ...
    def test_pic24f_memory_show_cli(self, pic24f_atpack_file):
        """Test memory show command for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["memory", "show", "PIC24F16KA301", str(pic24f_atpack_file)])
        
        assert result.exit_code == 0
        assert "PIC24F16KA301" in result.stdout
        assert "Memory Layout" in result.stdout

    def test_pic24f_memory_show_json_cli(self, pic24f_atpack_file):
        """Test memory show command with JSON output for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["memory", "show", "PIC24F16KA301", str(pic24f_atpack_file), "--format", "json"])
    ...
    def test_pic24f_registers_list_cli(self, pic24f_atpack_file):
        """Test registers list command for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["registers", "list", "PIC24F16KA301", str(pic24f_atpack_file)])
        
        assert result.exit_code == 0
        assert "PIC24F16KA301" in result.stdout
        assert "Registers" in result.stdout

    def test_pic24f_registers_list_json_cli(self, pic24f_atpack_file):
        """Test registers list command with JSON output for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["registers", "list", "PIC24F16KA301", str(pic24f_atpack_file), "--format", "json"])
    ...
    def test_pic24f_config_show_cli(self, pic24f_atpack_file):
        """Test config show command for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["config", "show", "PIC24F16KA301", str(pic24f_atpack_file)])
    ...
    def test_pic24f_config_show_json_cli(self, pic24f_atpack_file):
        """Test config show command with JSON output for PIC24F16KA301."""
        runner = CliRunner()
        result = runner.invoke(app, ["config", "show", "PIC24F16KA301", str(pic24f_atpack_file), "--format", "json"])
    ...
    def test_pic24f_cli_with_output_export(self, pic24f_atpack_file):
        ...
        try:
            result = runner.invoke(app, [
                "devices", "list", str(pic24f_atpack_file), 
                "--format", "json", 
                "--output", str(tmp_path)
            ])
    ...
    def test_pic24f_cli_no_color_option(self, pic24f_atpack_file):
        """Test CLI --no-color option with PIC24F devices."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "list", str(pic24f_atpack_file), "--no-color"])
...
class TestPIC24FEdgeCases:
    ...
    def test_pic24f_invalid_device_cli(self, pic24f_atpack_file):
        """Test CLI behavior with invalid PIC24F device name."""
        runner = CliRunner()
        result = runner.invoke(app, ["devices", "info", "INVALID_PIC24F", str(pic24f_atpack_file)])
        
        assert result.exit_code == 1
        assert "not found" in result.stdout.lower()

    def test_pic24f_invalid_register_cli(self, pic24f_atpack_file):
        """Test CLI behavior with invalid register name for PIC24F device."""
        runner = CliRunner()
        result = runner.invoke(app, ["registers", "show", "PIC24F16KA301", "INVALID_REG", str(pic24f_atpack_file)])
        
        assert result.exit_code == 1
        assert "not found" in result.stdout.lower()

    def test_pic24f_memory_segment_filter(self, pic24f_atpack_file):
...
class TestPIC24FComprehensive:
    ...
    def test_pic24f_cli_help_commands(self, pic24f_atpack_file):
    ...
    def test_pic24f_scan_functionality(self, pic24f_atpack_file):
        """Test directory scanning with PIC24F AtPack."""
        runner = CliRunner()
        atpack_dir = pic24f_atpack_file.parent
        
        result = runner.invoke(app, ["scan", str(atpack_dir)])
        assert result.exit_code == 0
        assert "PIC24F-KA-KL-KM_DFP" in result.stdout or "AtPack Files" in result.stdout

    def test_pic24f_all_cli_commands_basic(self, pic24f_atpack_file):
        """Test that all major CLI commands work for PIC24F without crashing."""
        runner = CliRunner()
        commands_to_test = [
            ["files", "info", str(pic24f_atpack_file)],
            ["devices", "list", str(pic24f_atpack_file)],
            ["devices", "info", "PIC24F16KA301", str(pic24f_atpack_file)],
            ["memory", "show", "PIC24F16KA301", str(pic24f_atpack_file)],
            ["registers", "list", "PIC24F16KA301", str(pic24f_atpack_file)],
            ["config", "show", "PIC24F16KA301", str(pic24f_atpack_file)],
        ]
